...
const Attendance = () => {
  ...
  return (
    <div className={`min-h-full bg-slate-50 transition-all duration-300 pt-28 sm:pt-32 md:pt-36 w-full ${contentClasses}`}>
      <div className="w-full h-full p-2 sm:p-3 md:p-4 lg:p-6">
        ...
        {/* Main Content Card */}
        <div className="bg-white rounded-lg sm:rounded-xl shadow-lg p-3 sm:p-4 md:p-6 lg:p-8 border border-slate-200 w-full">
          <h2 className="text-sm sm:text-base md:text-xl lg:text-2xl font-bold text-slate-800 mb-3 sm:mb-4 md:mb-6 flex items-center gap-2">
            <FaCalendarCheck className="text-base sm:text-lg md:text-xl text-slate-500 flex-shrink-0" /> 
          ...
          <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 md:gap-6 w-full">
            ...
            <div className="w-full lg:w-96 flex flex-col gap-2 sm:gap-3">
              <div className="flex items-center gap-2 bg-white px-3 py-2.5 sm:py-3 rounded-lg sm:rounded-xl shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
              ...
              <div className="mt-3 sm:mt-4">
                <h3 className="text-xs sm:text-sm md:text-base font-semibold text-slate-700 mb-2 sm:mb-3">Attendance Distribution</h3>
                <div className="flex items-end justify-around gap-2 sm:gap-3 h-20 sm:h-28 md:h-36 bg-slate-50 rounded-lg p-2 sm:p-3 border border-slate-200">
            ...
            <div className="w-full lg:flex-1">
              ...
              <div className="mb-3 sm:mb-4 flex flex-wrap gap-1.5 sm:gap-2 items-center">
                <FaFilter className="text-slate-500 text-sm sm:text-base" />
                <button onClick={()=>setFilter('All')} className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded text-[10px] sm:text-xs md:text-sm font-bold transition shadow-sm hover:shadow-md min-h-[36px] ${filter==='All'?'bg-slate-700 text-white shadow-md':'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>All</button>
              ...
              <div className="overflow-x-auto rounded-lg border border-slate-200 -mx-1 sm:mx-0">
                <table className="min-w-full bg-white">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-left text-slate-800 font-bold text-xs sm:text-sm md:text-base border-b border-slate-200">Date</th>
                      <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-left text-slate-800 font-bold text-xs sm:text-sm md:text-base border-b border-slate-200">Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {attendance.length === 0 ? (
                      ...
                      attendance
                        ...
                        .map((rec, idx) => (
                          <tr key={idx} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${rec.status === 'Present' ? 'bg-green-50/20' : rec.status==='Late' ? 'bg-yellow-50/20' : 'bg-red-50/20'}`}>
