  ...
  return (
    <>
      ...
      {loadingActivities && (
        ...
        >
          <ClipLoader size={80} color="#FBBF24" loading={true} />
          <p className="mt-4 text-yellow-300 text-xl font-semibold">
            Loading activities...
          </p>
        </div>
      )}
      ...
      <div className="min-h-screen w-430 bg-[#FFDAB9] p-6 flex justify-center items-start">
        <div className="max-w-4xl w-full bg-white rounded shadow p-6">
          ...
          {!loadingActivities && errorActivities && (
            <p className="text-center p-4 text-red-600">{errorActivities}</p>
          )}

          {!loadingActivities && activities.length === 0 && (
            <p className="text-center text-gray-900 p-4">
                ...
                >
                  ...
                  <div className="flex justify-end space-x-4 pt-4 border-t border-gray-200">
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="px-6 py-2 rounded-md bg-yellow-500 hover:bg-yellow-600 text-white font-semibold transition disabled:opacity-50 flex items-center justify-center space-x-2"
                    >
                      {submitting && (
                        <ClipLoader size={20} color="#fff" loading={true} />
                      )}
                      <span>{submitting ? "Submitting..." : "Submit Activity"}</span>
                    </button>
                  </div>
                </form>
              </div>
