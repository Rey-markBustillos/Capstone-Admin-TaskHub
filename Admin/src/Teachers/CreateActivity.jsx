...
const CreateActivity = () => {
  ...
  const fetchClassData = useCallback(async () => {
    ...
    try {
      const [classRes, activitiesRes] = await Promise.all([
        // AYOS: Binago mula sa /classes/ patungong /class/ para makuha ang specific na class
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append('classId', classId); // Use classId from URL
      ...
      fetchClassData(); // Re-fetch activities for the class
      setTimeout(() => {
        setSuccess('');
        closeCreateModal();
      }, 1500);
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-800 dark:text-gray-100 text-center sm:text-left">
            Activities for <span className="text-indigo-600 dark:text-indigo-400">{className}</span>
          </h2>
          <button
            ...
            <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] flex flex-col">
              <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
                <h3 className="text-xl sm:text-2xl font-semibold text-gray-800 dark:text-gray-100 flex items-center">
                  <FaPlusCircle className="mr-3 text-indigo-600 dark:text-indigo-400" /> Create Activity for {className}
                </h3>
                <button onClick={closeCreateModal} className="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 transition-colors" aria-label="Close modal">
                  <FaTimes size={20} />
                </button>
              </div>
              ...
              <div className="overflow-y-auto flex-grow pr-2 space-y-5">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div>
                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300" htmlFor="title-modal">Title <span className="text-red-500">*</span></label>
                    <input
                      className="w-full border border-gray-300 dark:border-gray-600 rounded-lg p-3 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300" htmlFor="description-modal">Description</label>
                    <textarea
                      className="w-full border border-gray-300 dark:border-gray-600 rounded-lg p-3 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300" htmlFor="date-modal">Deadline <span className="text-red-500">*</span></label>
                      <input
                        className="w-full border border-gray-300 dark:border-gray-600 rounded-lg p-3 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300" htmlFor="score-modal">Max Score</label>
                      <input
                        className="w-full border border-gray-300 dark:border-gray-600 rounded-lg p-3 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300" htmlFor="attachment-modal">Attachment (Optional)</label>
                    <input
                  ...
                  <div className="pt-4 mt-2 border-t border-gray-200 dark:border-gray-700">
                    <button
                      className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-150 flex items-center justify-center"
                      type="submit"
                    >
        ...
        <div className="w-full bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-2xl">
          <h3 className="sticky top-0 z-10 bg-white dark:bg-gray-800 py-3 text-2xl font-semibold mb-6 text-center text-gray-800 dark:text-gray-100 flex items-center justify-center">
            <FaListOl className="mr-3 text-indigo-600 dark:text-indigo-400" /> Activities List
          </h3>
          <div className="overflow-y-auto max-h-[calc(100vh-22rem)]">
            ...
            {!loadingActivities && !activitiesError && activitiesList.length === 0 && (
              <div className="text-center py-10 text-gray-500 dark:text-gray-400">
                ...
                <p className="text-lg">No activities found for this class.</p>
                <p className="text-sm">Click the "Create New Activity" button to add one.</p>
              </div>
            )}
            ...
            {activitiesList.length > 0 && (
              <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                <table className="min-w-full table-auto text-sm">
                  <thead className="bg-gray-100 dark:bg-gray-700/50 text-gray-700 dark:text-gray-300 uppercase sticky top-0 z-[5]">
                    <tr>
                    ...
                    {activitiesList.map((activity) => (
                      <tr key={activity._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors duration-150">
                        ...
                        <td className="px-5 py-4 text-center whitespace-nowrap">
                          {activity.attachment ? (
                            <a href={`http://localhost:5000${activity.attachment}`} target="_blank" rel="noopener noreferrer" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 hover:underline inline-flex items-center">
