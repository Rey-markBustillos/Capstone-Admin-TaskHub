...
const CreateActivity = () => {
  ...
  const fetchClassData = useCallback(async () => {
    ...
    try {
      const [classRes, activitiesRes] = await Promise.all([
        axios.get(`https://capstone-admin-task-hub-9c3u-p6r5s7bf2.vercel.app/api/class/${classId}`),
        axios.get(`https://capstone-admin-task-hub-9c3u-p6r5s7bf2.vercel.app/api/activities?classId=${classId}`)
      ]);
  ...
  await axios.post('https://capstone-admin-task-hub-9c3u-p6r5s7bf2.vercel.app/api/activities', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
