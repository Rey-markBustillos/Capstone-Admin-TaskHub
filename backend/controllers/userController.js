...
exports.createUser = async (req, res) => {
  try {
   const { name, email, password, role, lrn, teacherId, adminId, address, age, schoolName, useDefaultStudentPassword } = req.body;
   ...
   const requiresPersonalDetails = role === "student" || role === "teacher";
   const requiresSchoolName = role === "student";

    if (!name || !email || !role || (role === "student" ? (!normalizedLrn || (!password && !useDefaultStudentPassword)) : !password) ||
      (requiresPersonalDetails && (!normalizedAddress || !Number.isInteger(normalizedAge) || normalizedAge < 1)) ||
      (requiresSchoolName && !normalizedSchoolName)) {
      const message = requiresPersonalDetails
        ? `Name, email, ${role === "student" ? "password, LRN, " : "password, "}address, age${requiresSchoolName ? ', and name of school' : ''} are required for ${role}s`
    ...
    // Landing-page registrations use the password chosen by the student. Only
    // admin-created/imported students receive the generated LRN-based password.
    const accountPassword = role === "student" && useDefaultStudentPassword ? studentPassword : password;
    ...
    const user = new User({
      ...
      address: requiresPersonalDetails ? normalizedAddress : null,
      age: requiresPersonalDetails ? normalizedAge : null,
      schoolName: requiresSchoolName ? normalizedSchoolName : null,
      active: true,
    });
