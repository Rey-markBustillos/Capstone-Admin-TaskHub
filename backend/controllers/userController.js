   ...
   const requiresSchoolName = role === "student";
    ...
    if (!name || !email || !role || (role === "student" ? !normalizedLrn : !password) ||
      ...
      const message = requiresPersonalDetails
        ? `Name, email, ${role === "student" ? "LRN, " : "password, "}address, age${requiresSchoolName ? ', and name of school' : ''} are required for ${role}s`
    ...
    const hashedPassword = await bcrypt.hash(role === "student" ? studentPassword : password, 10);
    const user = new User({
      name,
      email: email.toLowerCase(),
      // Student credentials are always generated by the system. This prevents
      // an import or add form from needing (or overriding) a password column.
